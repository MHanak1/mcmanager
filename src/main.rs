...
async fn main() -> Result<()> {
    ...
    if first_launch {
...

        {
            ...
            std::io::stdin().read_line(&mut username).expect("Failed to read STDIN");

            print!("Enter the password for the administrator account.\nPassword: ");
            std::io::stdout().flush().expect("Failed to flush stdout");
            let mut password = String::new();
            std::io::stdin().read_line(&mut password).expect("Failed to read STDIN");
            print!("Confirm password: ");
            std::io::stdout().flush().expect("Failed to flush stdout");
            let mut confirm_password = String::new();
            std::io::stdin().read_line(&mut confirm_password).expect("Failed to read STDIN");
            ...
            let default_group = {
                ...
                Group {
                    ...
                    storage_limit: None,
                    config_blacklist: vec![String::from("online-mode"), String::from("server-port")],
            ...
            database.insert(&default_group, None).expect("Failed to insert default user group");
            database.insert(&admin_group, None).expect("Failed to insert administrator group");

            database.create_user_from(User {
                id: Default::default(),
            ...
            let mut config_file = File::create(&util::dirs::base_dir().join("config.toml")).expect("failed to create the config file");
            config_file
                .write_all(include_str!("resources/default_config.toml").replace("$default_group_id", default_group.id.to_string().as_str()).as_bytes())
                .expect("failed to write default config file");

            let mut velocity_config_file = File::create(&util::dirs::base_dir().join("velocity_config.toml")).expect("failed to create the velocity config file");
        ...
        println!("MCManager set up successfully. Now you should edit the config and restart this executable");
...
async fn try_download_velocity() -> Result<()> {
    ...
    let velocity: Builds = serde_json::from_str(
        &client.get(format!("https://api.papermc.io/v2/projects/velocity/versions/{}/builds", version).as_str())
            .send().await?
            .text().await?
    )?;
    let build = velocity.builds.last().unwrap();

    info!("Downloading the latest version of velocity: {}", build.downloads.application.name);

    let response = reqwest::get(format!("https://api.papermc.io/v2/projects/velocity/versions/{}/builds/{}/downloads/{}", version, build.build, build.downloads.application.name)).await?;
    let dir = util::dirs::velocity_dir().join("velocity.jar");
    let mut dest = File::create(&dir)?;
    let content =  response.bytes().await?;
    dest.write_all(&content)?;
    info!("Successfully downloaded the latest version of velocity to {}", dir.display());
